...
if ($deptId <= 0) {
    setFlashMessage('error', 'Invalid department ID');
    redirect('index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle image upload
    $imagePath = $dept['image']; // Keep existing image by default
    
    // Check if user wants to remove current image
    if (isset($_POST['remove_image']) && $_POST['remove_image'] == '1') {
        if (!empty($dept['image'])) {
            $fullPath = '../../' . ltrim($dept['image'], '/');
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE departments SET 
                    name = ?, image = ?, head_name = ?, description = ?, 
                    email = ?, phone = ?, display_order = ?, is_active = ?
                    WHERE department_id = ?";
            $stmt = $db->prepare($sql);
            $result = $stmt->execute([$name, $imagePath, $headName, $description, $email, $phone, $displayOrder, $isActive, $deptId]);
            
            if ($result) {
                logActivity($_SESSION['admin_id'], 'update', 'departments', $deptId, "Updated department: {$name}");
...
<div class="page-header">
    <div>
        ...
        <p class="subtitle">Update department information</p>
    </div>
                    ...
                    <div class="image-upload-wrapper">
                        <div class="image-upload-area" id="imageUploadArea">
                            ...
                            <div class="image-preview" id="imagePreview">
                                <img src="<?php echo escapeHtml($dept['image']); ?>" alt="Current image" id="previewImage">
                            ...
                            <div class="image-preview" id="imagePreview" style="display: none;">
                                ...
                                <button type="button" class="remove-image" id="removeImageNewUpload">
                                    <i class="fas fa-times"></i>
                                </button>
                            </div>
                            <?php endif; ?>
        ...
        <div class="form-sidebar">
            ...
            <div class="form-card">
                <div class="form-actions">
                    ...
                    <a href="delete.php?id=<?php echo $deptId; ?>" class="btn btn-danger btn-block">
...
document.getElementById('removeImage').addEventListener('click', function(e) {
    ...
    uploadArea.innerHTML = '<div class="upload-placeholder"><i class="fas fa-cloud-upload-alt"></i><p>Click to upload or drag and drop</p><small>JPG, PNG, GIF or WebP (Max 5MB)</small></div><input type="file" id="image" name="image" accept="image/*" class="file-input">';
    setupNewUpload();
});
...
function setupNewUpload() {
    ...
    newFileInput.addEventListener('change', function() {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                ...
                newUploadArea.querySelector('.upload-placeholder').style.display = 'none';
    ...
    const removeNewBtn = document.getElementById('removeImageNewUpload');
    if (removeNewBtn) {
        removeNewBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            newFileInput.value = '';
            newUploadArea.querySelector('.upload-placeholder').style.display = 'flex';
...
.image-upload-area{position:relative;border:2px dashed var(--admin-border);border-radius:8px;padding:2rem;text-align:center;cursor:pointer;transition:all 0.3s;min-height:200px;display:flex;align-items:center;justify-content:center}
...
.image-preview img{width:100%;max-height:300px;object-fit:contain;border-radius:8px}
.remove-image{position:absolute;top:10px;right:10px;width:32px;height:32px;background:var(--admin-danger);color:white;border:none;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:10}
.remove-image:hover{background:#c82333}
.btn-change-image{padding:0.5rem 1rem;background:var(--admin-primary);color:white;border:none;border-radius:6px;font-size:0.85rem;cursor:pointer;display:inline-flex;align-items:center;gap:0.5rem;transition:all 0.3s}
.btn-change-image:hover{background:var(--admin-primary-dark)}
